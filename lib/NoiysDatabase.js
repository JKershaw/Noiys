var NoiysDatabase = function(connection_string) {
		...
		function findStatus(statusID, callback) {
			try {
				if (status_cache[statusID] && (status_cache[statusID] !== undefined)) {
				 	console.log(".");
				 	callback(status_cache[statusID]);
				} else {
		...
		function getStatusesFromIDs(statusIDs, callback) {
			...
			if (statusIDs.length <= 10) {
				...
				getStatuses(function(statuses){
					...
					for (var i = 0; i < statuses.length; i++) {
						if (statusIDs.indexOf(statuses[i].id) != -1){
							wanted_statuses.push(statuses[i]);
						}
					}

					callback(wanted_statuses);
