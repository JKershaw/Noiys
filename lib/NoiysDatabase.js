		...
		try {
			...
			db.statuses.find(query).toArray(function(err, statuses) {
				if (statuses && statuses[0]) {
					...
					if (status.text.length > 10000) {
						status.text = status.text.substr(0, 10000) + " <br /><strong>[...]</strong>";
					}
				} else {
					status = false;
				}
	...
	function getStatuses(callback) {
		db.statuses.find({}).limit(1000).toArray(function(err, statuses) {
			...
			for (var i = 0; i < statuses.length; i++) {
				...
				if (statuses[i].text.length > 10000) {
					statuses[i].text = statuses[i].text.substr(0, 10000) + " <br /><strong>[...]</strong>";
				}
			}
			callback(statuses);
