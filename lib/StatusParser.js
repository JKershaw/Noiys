		...
		if (quotes !== null) {
			...
			_.each(quotes, function(quote) {
				...
				get_status_text(quoted_status_id, function(status, original_id) {
					...
					var embedded_quote = "<div " + found_quoted_status_votes_string + " data-id=\"" + original_id + "\" class=\"panel panel-default\"><div class=\"panel-body\">" + found_quoted_status_text + "</div></div>";
...
function get_status_text(id, callback) {
	...
	noiysDatabase.findStatus(id, function(status) {
		...
		if (!status) {
			status.text = "<i>Status not found</i>";
			status.id = id;
		}
