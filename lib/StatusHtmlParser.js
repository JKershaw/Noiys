...
var StatusHtmlParser = function(status_text, callback) {
		...
		parse_quote(status_text, 0, function(status_text) {
			parse_link(status_text, function(status_text) {
				parse_nl2br(status_text, function(status_text) {
					...
					status_html = process_hashtags(status_text);
					status_html = " \
					<div class=\"panel-body\"> \
						" + status_html + " \
					</div>";

					callback(status_text, status_html);
				});
			});
		});
	}
	...
	noiysDatabase.findStatus(id, function(status) {
		...
		if (!status) {
			...
			var oldNoteMessages = [
				...
				"Note deleted.",
				"Note expired.",
				"Your Note is in another castle.",
				"Note traversed to the void.",
				"Note eliminated.",
				"This Note's life clock has expired.",
				"Too late; Note died of old age.",
				"Like a mayfly, this Note has lived a brief, bright life, and has now moved on.",
				"Note tried to swim in lava.",
				"Note shot by skeleton.",
				"This Note is dead, Dave.",
				"Note recycled.",
				"Note valiantly fell in battle.",
				"Note deaded.",
				"Note no more.",
				"Note not.",
				"Note dissolved.",
				"Note lost to the universe.",
				"Note has been dispatched by digital ninjas.",
				"Note was eaten by a grue.",
				"Note taped over.",
				"Note exploded.",
				"Note incinerated.",
				"Note imploded.",
				"Note evaporated.",
				"Note dismantled for scrap.",
				"Note parts being reused.",
				"Note consumed iocane powder, INCONCEIVABLE!",
				"Note was banished to Carmarthen.",
				"Note was left in a communal fridge by a guest, went off, and was eventually thrown out by someone's mum.",
				"Note's passed on! This note is no more! He has ceased to be! Note's expired and gone to meet 'is maker! Note's a stiff! Bereft of life, 'e rests in peace! If you hadn't nailed 'im to the perch 'e'd be pushing up the daisies! Note's metabolic processes are now 'istory! Note's off the twig! 'Note's kicked the bucket, Note's shuffled off 'is mortal coil, run down the curtain and joined the bleedin' choir invisible!! THIS IS AN EX-NOTE!",
				"Note has journeyed to Avalon.",
				"Note has ascended to Valhalla.",
				"Note spoke loudly about a conspiracy theory in range of a man in dark glasses and has not been seen since."];
