		...
		parse_text(status_text,  function(status_text) {
			...
			if (status.parent) {
				get_status_inc_deleted(status.parent, function(parent_status){
					parse_prior_conversation(parent_status, function(parent_html) {
...
function parse_prior_conversation(status, callback) {
	...
	parse_text(status.text,  function(status_text) {
		...
		if (!status.parent) {
			current_html = "<li class=\"list-group-item list-status\">" + status_text + "</li>";
			...
			get_status_inc_deleted(status.parent, function(parent_status){
				parse_prior_conversation(parent_status, function(parent_html){
					current_html = parent_html + "<li class=\"list-group-item list-status\">" + status_text + "</li>";
					status.responses = undefined;
					current_html = current_html + noiysUi.generate_icon_row_html(status, true);
					callback(current_html);
				});
